    ...
    .then(function (value) {
      ...
      addToBasket.addEventListener("click", function () {
        ...
        const popup = () => {
          if (
            window.confirm(
              `Vous avez ajouté au panier  ${selectionClient.quantity} ${selectionClient.name} avec une lentille de ${selectionClient.option} pour un total de ${selectionClient.price}. Voulez vous retouner sur la page d'accueil ou voir votre panier? OK pour voir votre panier, ANNULER pour retourner sur la page d'accueil `
            )
          ) {
  ...
  if (localStorage.product) {
    ...
    submit.addEventListener("click", function () {
      ...
      for (let i = 0; i < test.length; i++) {
        let id = JSON.parse(localStorage.getItem("product"))[i].id;
      ...
      fetch("http://localhost:3000/api/cameras/order/", {
        ...
        .then((value) => {
          submit.setAttribute("href", "confirm.html?order=" + value.orderId); 
        });
    });
  }
}
...
function confirmOrder(){
  ...
  confirm.textContent = `Votre commande n° ${location.search.substring(7)} a bien été validé. Vous recevrez un mail de confirmation dans quelques instants`
  localStorage.clear();
}
